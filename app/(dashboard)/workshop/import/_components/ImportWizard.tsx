...
import { importBoqAction } from "@/app/(dashboard)/workshop/import/actions";
...
export function ImportWizard({
  ...
  const applyPreviewResult = useCallback((data: ExcelPreviewDto) => {
    setPreview(data);
    setAllRows(data.allRows);
    setColumnMappingByIndex(
      buildColumnMappingByIndex(data.headers, inferWizardColumnMapping(data.headers)),
    );
  }, []);
  ...
  const loadExcelFile = useCallback(
    async (file: File, sheetName?: string) => {
      ...
      try {
        ...
        notify(`Loaded ${data.totalRowCount} rows from "${data.sheetName}"`, "success");
        return true;
      } catch (error) {
  ...
  const mappingComplete = WIZARD_FIELD_ORDER.every((field) => assignments.has(field));

  const requiredFieldStatus = useMemo(
    () =>
      WIZARD_FIELD_ORDER.map((field) => ({
        key: field,
    ...
    <ShellContent className="mx-auto w-full min-w-0 max-w-7xl pb-10">
      <div className="mb-4 space-y-1">
        ...
        <Text variant="muted" size="sm">
          Upload an Excel file, map four columns, then preview before import.
        </Text>
      </div>
      ...
      <div className={cn("flex flex-col", WIZARD_GAP)}>
        <div className={cn("grid grid-cols-1 lg:grid-cols-2 lg:items-start", WIZARD_GAP)}>
          ...
          <ImportWizardStep
            ...
            tone="amber"
            title="Map columns"
            description="Match each Excel column to Item number, Description, Unit, or Quantity."
        ...
        {preview && mappedPreview ? (
          <ImportWizardStep
            ...
            description="Only mapped system fields are shown — not every Excel column."
            ...
            <div className="flex flex-col gap-3 border-t border-emerald-200 pt-4 sm:flex-row sm:items-center">
              ...
              >
                {importing ? (
                  ...
                  "Import and start categorization"
                )}
              </Button>
              <Text variant="muted" size="sm">
