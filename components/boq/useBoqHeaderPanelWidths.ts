...
const STORAGE_KEY = "boq-breakdown-header-panel-widths-v2";
const RESIZER_COUNT = 4;
export const BOQ_HEADER_RESIZER_WIDTH = 5;
const MIN_PANEL_WIDTH = 72;

/** Default panel widths (px) — Discipline | Project | Approval | Summary | Edits */
export const BOQ_HEADER_DEFAULT_WIDTHS = [164, 208, 248, 328, 212] as const;

function loadStoredWidths(totalContentWidth: number): number[] {
  const defaults = scaleWidths(BOQ_HEADER_DEFAULT_WIDTHS, totalContentWidth);
  ...
  try {
    ...
    if (
      parsed.length !== BOQ_HEADER_DEFAULT_WIDTHS.length ||
...
export function useBoqHeaderPanelWidths() {
  ...
  const measureAndInit = useCallback(() => {
    ...
    const totalContentWidth = Math.max(
      container.clientWidth - RESIZER_COUNT * BOQ_HEADER_RESIZER_WIDTH,
      MIN_PANEL_WIDTH * BOQ_HEADER_DEFAULT_WIDTHS.length,
    );
  ...
  return {
    ...
    widths,
    startResize,
    resizerWidth: BOQ_HEADER_RESIZER_WIDTH,
  };
}
